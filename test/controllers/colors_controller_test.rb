...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create color' do
    assert_difference('Color.count') do
      post colors_url, params: { color: { description: @color.description, name: @color.name } }
    end
  ...
  test 'should update color' do
    patch color_url(@color), params: { color: { description: @color.description, name: @color.name } }
